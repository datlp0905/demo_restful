...
//    private EntityManager em;
    ...
    @GET
    @Path("searchByName/{search}")
    @Produces(MediaType.APPLICATION_XML)
    public List<TblProduct> findBuLikeName(@PathParam("search") String search) {
        return super.searchByLikeName(search);
    }
    
    @GET
    @Path("searchByPrice/{from}/{to}")
    @Produces(MediaType.APPLICATION_XML)
    public List<TblProduct> findByPrice(@PathParam("from") float from, @PathParam("to") float to) {
        return super.searchByPriceInRange(from, to);
    }
